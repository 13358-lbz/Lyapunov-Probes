...
def per_model_scores(path, lama_cleanup=False, force_check_labels_path=None, csv_filename="popqa_sf_1000_None.csv"):
    ...
    for folder in sorted(folders):
        ...
        if force_check_labels_path is not None:
            if "tensor" in uuids[0].__str__():
                ...
                def parse_uuid(uuid):
                    ...
                    if isinstance(uuid, str):
                        if uuid.startswith("tensor(") and uuid.endswith(")"):
                            return int(uuid[len("tensor("):-1])
                        else:
                            return int(uuid)
                    # 如果是tensor对象
                    elif hasattr(uuid, "item"):
                        return uuid.item()
                    # 如果是普通数字
                    elif isinstance(uuid, (int, float)):
                        return int(uuid)
                    else:
                        raise ValueError(f"Unknown uuid format: {uuid}")

                # 用法示例
                uuids = [parse_uuid(u) for u in uuids]
            ...
            # gen_data["uuid"] = [int(uuid) for uuid in gen_data["uuid"]]
...
def main(
    results_path: str = "results_pik2_triviaqa_soft/",
    force_check_labels_path: str = "results_kn_triviaqa/",
    csv_filename: str = "popqa_sf_1000_None.csv"
):
    scores, corrs = per_model_scores(
        results_path, force_check_labels_path=force_check_labels_path, csv_filename=csv_filename
    )
    print(scores)
    print("correlations :")
