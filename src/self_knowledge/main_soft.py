...
from self_knowledge.evaluation.train_scorer_soft import MLP2

accelerator = Accelerator()


def main(
    model_name: str = "download_models/falcon-7b-instruct",
    ...
    if "hidden" in methods or "value" in methods:
        # 判断MLP2从哪个库里面来
        if MLP2.__module__ == "self_knowledge.evaluation.train_scorer":
            logging.info("Using MLP2 from train_scorer.py")
            hidden_scorer = MLP2(hidden_size=model.config.hidden_size).to(
                accelerator.device
            )
        elif MLP2.__module__ == "self_knowledge.evaluation.train_scorer_soft":
            logging.info("Using MLP2 from train_scorer_soft.py")
            # soft delta
            hidden_scorer = MLP2(hidden_size=model.config.hidden_size + 1).to(
                accelerator.device
            )
    ...
    with torch.no_grad():
        for batch_idx, batch in enumerate(tqdm(dataloader)):
            new_scores = run_scoring(
                ...
                # batch["question"] if not paraphrase else batch["paraphrase"],
                uuids=batch["uuid"],
                # uuids=batch["key"],
