...
def main(
    hidden_path: str = "models/lama3_hlayer_24_test.pt",
    out_name: str = "models/lama_hidden_scorer/download_models/falcon-7b-instruct/lama3_hscorer_test_24.pt",
    seed: int = 42,
):
    ...
    for item in dataset:
        ...
        if isinstance(val, torch.Tensor):
            item["is_factual"] = val.float() if val.dtype != torch.float32 else val
        else:
            item["is_factual"] = torch.tensor(float(val))
