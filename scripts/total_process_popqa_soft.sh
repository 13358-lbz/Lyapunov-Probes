for perturb_times in 3; do
    for perturb_prob in 0.30; do
        ...
        for i in "${!models[@]}"; do
            ...
            (
                ...
                CUDA_VISIBLE_DEVICES=$gpu python -m self_knowledge.evaluation.extract_hidden_layers_temp \
                    ...
                    --save-file="models/$model/lama3_popqa_soft_hlayer_$layer.pt"
                ...
                CUDA_VISIBLE_DEVICES=$gpu python -m self_knowledge.evaluation.train_scorer_soft \
                    --hidden-path="models/$model/lama3_popqa_soft_hlayer_$layer.pt" \
                    --out-name="models/lama_hidden_scorer/$model/lama3_popqa_hscorer_soft_$layer.pt"
            ) &
        done
        ...
        for i in "${!models[@]}"; do
            ...
            (
                CUDA_VISIBLE_DEVICES=$gpu python src/self_knowledge/main_soft.py \
                    ...
                    --hidden-scorer-path="models/lama_hidden_scorer/$model/lama3_popqa_hscorer_soft_24.pt" \
                    --save-path="results_pik2_popqa_soft/$model/" \
                    --batch-size=32 \
        ...
        python src/self_knowledge/graphing/draw_graphs.py \
            --results-path="results_pik2_popqa_soft/" \
