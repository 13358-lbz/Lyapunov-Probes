...
for model_name in "${model_names[@]}"; do
    ...
    for i in 24 -1; do
        echo $model_name
        echo $i
        python -m self_knowledge.evaluation.extract_hidden_layers --model-name=$model_name --hidden-layer-index=$i --save-file=../models/$model_name/lama3_hlayer_$i.pt --dataset-path=../data/clean_popQA.csv
        # python -m self_knowledge.evaluation.train_scorer --hidden-path=../models/$model_name/lama3_hlayer_$i.pt --out-name=../models/lama_hidden_scorer/$model_name/lama3_hscorer_$i
    done
done